...
public class FulfillmentsController {
  ...
  ResponseEntity<String> fulfillmentPut(@PathVariable String id, @RequestBody FulfillmentPut params) {

    logger.info("received PUT request for id {}", id);
    var wfStub = temporalClient.newWorkflowStub(Order.class, params.orderId());
    wfStub.completeProductFulfillment(new CompleteProductFulfillmentRequest(params.productType(), id, params.fulfillmentStatus()));
